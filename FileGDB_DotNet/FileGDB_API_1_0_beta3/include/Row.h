...
public:
  ...
  /// @param[in]   value The value to assign to the field.
  /// @return       Error code indicating whether the method finished successfully.
  fgdbError SetGUID(const std::wstring& field, GUID value);
...
private:
  ...
  fgdbError FindField(const std::wstring& field, long& fieldNumber, FieldType& fieldType);
  fgdbError GetFieldIsNullable(long fieldNumber, bool& isNullable);

  fgdbError SetOID(int32 objectID);
  fgdbError SetGlobalID(GUID globalID);
